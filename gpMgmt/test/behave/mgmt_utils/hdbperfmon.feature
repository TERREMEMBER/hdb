...
Feature: gpperfmon
    ...
    """
    @hdbperfmon_install_help
    Scenario: hdbperfmon help info
    When the user runs "hdbperfmon_install -?"
	 Then hdbperfmon_install should return a return code of 1
	 And hdbperfmon_install should print "COMMAND NAME: hdbperfmon_install" to stdout
	 And hdbperfmon_install should print "Creates the gpmon superuser role in inHybrid Database." to stdout
	 And hdbperfmon_install should print "--port <hdb_port>" to stdout
     And hdbperfmon_install should print "hdbperfmon_install --port 5432" to stdout
     And hdbperfmon_install should print "hdbperfmon_install --enable --password changeme --port 5432" to stdout
    
    @hdbmmon_version
    Scenario: hdbmmon version info
    When the user runs "hdbmmon --version"
	 Then hdbperfmon_install should return a return code of 0
	 And hdbperfmon_install should print "hdbmmon packet version 5" to stdout
    ...
    Scenario: gpperfmon keeps all partitions upon restart if partition_age not set, drops excess partitions otherwise
        ...
        When the user runs command "pkill hdbmmon"
