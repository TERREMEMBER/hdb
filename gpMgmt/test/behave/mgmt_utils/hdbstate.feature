@hdbstate
Feature: hdbstate tests

    Scenario: hdbstate -b logs cluster for a cluster where the mirrors failed over to primary
        Given a standard local demo cluster is running
        And the database is running
        When user stops all primary processes
        And user can start transactions
        And the user runs "hdbstate -b"
    ...
    Scenario: hdbstate -c logs cluster info for a cluster where all mirrors are failed over
        Given a standard local demo cluster is running
        And the database is running
        When user stops all primary processes
        And user can start transactions
        And the user runs "hdbstate -c"
        Then hdbstate output looks like
         ...
         And hdbstate should print "3 segment\(s\) configured as mirror\(s\) are acting as primaries" to stdout

    Scenario: hdbstate -c logs cluster info for a cluster that is unsynchronized
        Given a standard local demo cluster is running
        ...
        Then hdbstate output looks like
         ...
         And hdbstate should print "3 primary segment\(s\) are not synchronized" to stdout

    Scenario: hdbstate -c logs cluster info for a cluster with no mirrors
        Given the cluster is generated with "3" primaries only
        When the user runs "hdbstate -c"
        Then hdbstate should print "Primary list \[Mirror not used\]" to stdout
    ...
    Scenario: hdbstate -b logs cluster for a cluster without standbys
        Given the cluster is generated with "3" primaries only
        And the user runs "hdbstate -b"
        Then hdbstate output has rows with keys values
            | Master instance                              = Active                       |
    ...
    Scenario: hdbstate -e logs no errors when there are none
        Given a standard local demo cluster is running
        And the user runs "hdbstate -e"
        Then hdbstate should print "Segment Mirroring Status Report" to stdout
        And hdbstate should print "All segments are running normally" to stdout

    Scenario: hdbstate -e logs errors when mirrors have failed over
        Given a standard local demo cluster is running
          And user stops all primary processes
          And user can start transactions
        When the user runs "hdbstate -e"
        Then hdbstate should print "Segments with Primary and Mirror Roles Switched" to stdout
        And hdbstate output looks like
            | Current Primary | Port   | Mirror | Port   |
            | \S+             | [0-9]+ | \S+    | [0-9]+ |
            | \S+             | [0-9]+ | \S+    | [0-9]+ |
            | \S+             | [0-9]+ | \S+    | [0-9]+ |
        And hdbstate should print "Unsynchronized Segment Pairs" to stdout
        And hdbstate output looks like
            | Current Primary | Port   | Mirror | Port   |
            | \S+             | [0-9]+ | \S+    | [0-9]+ |
            | \S+             | [0-9]+ | \S+    | [0-9]+ |
            | \S+             | [0-9]+ | \S+    | [0-9]+ |
        And hdbstate should print "Downed Segments" to stdout
        And hdbstate output looks like
    ...
    Scenario: hdbstate -c logs cluster info for a mirrored cluster
        Given a standard local demo cluster is running
        When the user runs "hdbstate -c"
        Then hdbstate output looks like
            | Status                           | Data State   | Primary | Datadir                 | Port   | Mirror | Datadir                        | Port   |
    ...
    Scenario: hdbstate -b logs cluster for a default cluster
        Given a standard local demo cluster is running
        And the user runs "hdbstate -b"
        Then hdbstate output has rows with keys values
            | Master instance                                         = Active                       |
    ...
    Scenario: hdbstate -f logs master standyby details
        Given a standard local demo cluster is running
        When the user runs "hdbstate -f"
        Then hdbstate output has rows with keys values
    ...
    Scenario: hdbstate -m warns when mirrors have failed over to primary
        Given a standard local demo cluster is running
          And user stops all primary processes
          And user can start transactions
        When the user runs "hdbstate -m"
        ...
        And hdbstate should print "3 segment\(s\) configured as mirror\(s\) are acting as primaries" to stdout
        And hdbstate should print "3 mirror segment\(s\) acting as primaries are not synchronized" to stdout

    Scenario: hdbstate -p logs port details
        Given a standard local demo cluster is running
        When the user runs "hdbstate -p"
        Then hdbstate should print "Master segment instance .*/demoDataDir-1  port = .*" to stdout
        And hdbstate should print "Segment instance port assignments" to stdout
        And hdbstate output looks like
            | Host | Datadir                         | Port   |
            | \S+  | .*/dbfast1/demoDataDir0         | [0-9]+ |
    ...
    Scenario: hdbstate -s logs detailed information
        Given a standard local demo cluster is running
        When the user runs "hdbstate -s"
        Then hdbstate output has rows with keys values
    ...
    Scenario: hdbstate -i logs version info for all segments
        Given a standard local demo cluster is running
    ...
    Scenario: hdbstate -i warns if any up mirrors cannot be contacted
        Given a standard local demo cluster is running
          And user stops all mirror processes
    ...
    Scenario: hdbstate -x logs gpexpand status
        Given the cluster is generated with "3" primaries only
         When the user runs "hdbstate -x"
         Then hdbstate output looks like
