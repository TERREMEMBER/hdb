...
class MigratePackagesTestCase(GpTestCase):
    ...
    def test__execute_target_gphome_mismatch_raises(self):
        ...
        expected_raise = "The target HDBHOME, %s, must match the current \$HDBHOME used to launch hdbpkg." % self.args['to_gphome']
    ...
    def test__execute_identical_source_target_raises(self):
        ...
        expected_raise = "The source and target HDBHOMEs, %s => %s, must differ for packages to be migrated." % (self.args['from_gphome'], self.args['to_gphome'])
        with self.assertRaisesRegexp(ExceptionNoStackTraceNeeded, expected_raise):
            subject.execute()
